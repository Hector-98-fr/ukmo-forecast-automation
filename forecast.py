...
# =========================================================
# 1. CONFIG (ONLY THING YOU MAY EDIT)
# =========================================================

SHAPEFILE_PATH = "data/Gaza_adm2.shp"  # <-- moved to repo folder


# =========================================================
# 2. UKMO DATA PIPELINE (UNCHANGED LOGIC)
# =========================================================
...
def load_ukmo_data():
    ...
    search = catalog.search(collections=collections, max_items=100)
    ...
    latest_00z = [t for t in ref_times if "T00:00:00Z" in t][-1]
    print("Latest 00Z:", latest_00z)
...
# =========================================================
# 3. PROCESSING (YOUR ORIGINAL LOGIC CLEANLY WRAPPED)
# =========================================================

def process_forecast(datasets):

    temp_ds = [
        ds[['air_temperature']].sel(latitude=slice(30, 33), longitude=slice(33, 36))
        for ds in datasets if 'air_temperature' in ds.data_vars
    ]

    ws_ds = [
        ds[['wind_speed']].sel(latitude=slice(30, 33), longitude=slice(33, 36))
        for ds in datasets if 'wind_speed' in ds.data_vars
    ]

    wind_ds = [
        ds[['wind_speed_of_gust']].sel(latitude=slice(30, 33), longitude=slice(33, 36))
        for ds in datasets if 'wind_speed_of_gust' in ds.data_vars
    ]

    precip_ds = [
        ds[['lwe_precipitation_rate']].sel(latitude=slice(30, 33), longitude=slice(33, 36))
        for ds in datasets if 'lwe_precipitation_rate' in ds.data_vars
    ]
    ...
    ds_all = xr.merge([temp_all, ws_all, wind_all, precip_all], compat='override')
    ...
    ds_all['lwe_precipitation_rate'] = ds_all['lwe_precipitation_rate'] * 86400 * 1000

    ds_all = ds_all.sortby("time")

    daily_temp = ds_all["air_temperature"].resample(time="1D").max()
    daily_ws = ds_all["wind_speed"].resample(time="1D").max()
    daily_wind = ds_all["wind_speed_of_gust"].resample(time="1D").max()
    daily_precip = ds_all["lwe_precipitation_rate"].resample(time="1D").sum()
...
# =========================================================
# 4. SHAPEFILE FUNCTION (UNCHANGED)
# =========================================================

def calculate_governorate_tables(ds, shapefile_path, governorate_column='adm2_name'):
    ...
    for i in range(len(lats)):
        for j in range(len(lons)):
            grid_cells.append({
                'lat_idx': i,
                'lon_idx': j,
                'geometry': box(lon_edges[j], lat_edges[i],
                                lon_edges[j+1], lat_edges[i+1])
            })

    grid_gdf = gpd.GeoDataFrame(grid_cells, crs='EPSG:4326').to_crs(target_crs)
    ...
    for _, gov in gdf_proj.iterrows():
        ...
        intersections = grid_gdf.geometry.intersection(gov_geom)
    ...
    for g, gov_name in enumerate(gov_names):
        ...
        for t in range(len(times)):
            t_val = ws_val = w_val = p_val = 0

            for cell in cells:
                i, j = cell['lat_idx'], cell['lon_idx']
                f = cell['fraction']
    ...
    df_temp = pd.DataFrame(temp_out, index=times, columns=gov_names)
    df_ws = pd.DataFrame(ws_out, index=times, columns=gov_names)
    df_wind = pd.DataFrame(wind_out, index=times, columns=gov_names)
    df_precip = pd.DataFrame(precip_out, index=times, columns=gov_names)

    return df_temp, df_ws, df_wind, df_precip


# =========================================================
# 5. MAIN ENTRY POINT (IMPORTANT FOR GITHUB ACTIONS)
# =========================================================

def main():

    print("Starting UKMO pipeline...")
    ...
    print("Forecast tables generated successfully.")

    # Optional: quick check output
    print(df_temp.head())
